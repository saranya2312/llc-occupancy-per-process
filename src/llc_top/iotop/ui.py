...
class IOTopUI(object):
    ...
    def get_data(self):
	    ...
	    read_bytes = proct(str(p.pid))
            line = (pid_format + ' %4s  %-8s %20s  ') % (
    ...
    def refresh_display(self, first_time, total, actual, duration):
        ...
        if self.options.processes:
            ...
            pid += 'TID'
        titles = [pid, '  PRIO', '  USER', '                    LLC USAGE']
        lines = self.get_data()
        ...
        if self.options.batch:
            ...
            self.win.hline(0, 0, ord(' ') | curses.A_REVERSE,
                           self.width)
            ...
            for i in range(num_lines):
                try:
                    ...
                    pass
            if status_msg:
                self.win.insstr(self.height, 0, status_msg,
                                curses.A_BOLD)
            self.win.refresh()
